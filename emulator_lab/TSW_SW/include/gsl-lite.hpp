...
#undef gsl_not_null_explicit
	...
	template <class T, class U> std::ptrdiff_t operator-( not_null<T> const&, not_null<U> const& ) gsl_is_delete;
...
#define gsl_ENABLE_IF_INTEGRAL_T( T )
	...
	template <class T> gsl_api inline gsl_constexpr span<T> make_span( T * ptr, typename span<T>::index_type count )
	{
	...
	template <class T, size_t N> gsl_api inline gsl_constexpr span<const T> make_span( std::array<T, N> const& arr )
	{
